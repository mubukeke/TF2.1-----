...
for epoch in range(epoch):
    for batch, (x_train_batch, y_train_batch) in enumerate(train_db):
        with tf.GradientTape() as tape:
            ...
            # 添加 L2 正则项（再loss上对参数加惩罚项）
            loss_regularization = []
            loss_regularization.append(tf.nn.l2_loss(w1))
            loss_regularization.append(tf.nn.l2_loss(w2))
            loss_regularization = tf.reduce_sum(loss_regularization)  # 加上 L2_w1 L2_w2

            loss = loss_mse + 0.015 * loss_regularization   # REGULARIZATION = 0.03    调大，拟合变坏；调小，拟合变好
        # 对各个参数求梯度
